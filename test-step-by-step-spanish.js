...
async function testExportStepByStep() {
  ...
  try {
    ...
    processData.forEach((evaluacion, index) => {
      console.log(`   Procesando registro ${index + 1}:`, {
        id: evaluacion.id,
        alumno_id: evaluacion.alumno_id,
        hasAlumnos: !!evaluacion.alumnos,
        nombre: evaluacion.alumnos?.nombre
      })
      
      if (!evaluacion.alumnos) {
        console.error('❌ Registro sin datos de alumno:', evaluacion)
        return
      }
      ...
      if (!alumnosMap.has(alumnoId)) {
        alumnosMap.set(alumnoId, {
          ...
          promedio_diario: 0,
          dias_evaluados: 0,
          data_primeira_avaliacao: evaluacion.fecha,
          hora_primeira_avaliacao: ''
        })
      }
    })
    
    console.log(`✅ Procesamiento completado. ${alumnosMap.size} alumnos únicos`)
