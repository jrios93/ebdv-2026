-- ========================================
-- CONFIGURACIÓN SUPABASE REALTIME Y RLS
-- ========================================
...
-- 2. CREAR POLÍTICAS RLS (ROW LEVEL SECURITY)
...
CREATE POLICY "Los maestros pueden ver otros maestros" ON maestros
  FOR SELECT USING (rol = 'maestro' OR rol = 'jurado' OR rol = 'admin');

CREATE POLICY "Los maestros pueden ver classrooms" ON classrooms
  FOR SELECT USING (activo = true);

-- 4. POLÍTICAS PARA PUNTUACIÓN GRUPAL DIARIA
-- Lectura: Jurados pueden ver evaluaciones de su mismo rol
CREATE POLICY "Jurados pueden ver todas las evaluaciones" ON puntuacion_grupal_diaria
  FOR SELECT USING (
    EXISTS (
...
-- Escritura: Solo jurados pueden insertar sus propias evaluaciones
...
-- Actualización: Solo el jurado que creó puede modificar su evaluación
...
-- 5. CREAR FUNCIÓN PARA VERIFICAR SI UN JURADO PUEDE EVALUAR
...
-- 6. CREAR FUNCIÓN PARA OBTENER JURADOS DISPONIBLES
...
-- 7. CREAR FUNCIÓN PARA OBTENER EVALUACIONES DEL DÍA CON PROMEDIOS
...
-- 8. VERIFICACIÓN - Probar funciones
SELECT * FROM jurado_puede_evaluar(
  (SELECT id FROM maestros WHERE nombre LIKE 'Emilio%' LIMIT 1),
  (SELECT id FROM classrooms WHERE nombre = 'Luz' LIMIT 1),
  CURRENT_DATE
);

SELECT * FROM obtener_jurados_disponibles();

-- 9. VERIFICACIÓN - Ver políticas RLS
SELECT schemaname, tablename, rowsecurity 
FROM pg_tables 
WHERE tablename IN ('puntuacion_grupal_diaria', 'maestros', 'classrooms')
AND schemaname = 'public';

-- ========================================
-- CONFIGURACIÓN DE PUBLICATIONS PARA REALTIME
-- ========================================

-- 10. Asegurar que las tablas estén en la publication por defecto
-- (Generalmente ya está configurado en Supabase, pero lo verificamos)
...
-- ========================================
-- ESTE SCRIPT DEBE EJECUTARSE DESPUÉS DEL SCRIPT 001
-- ========================================